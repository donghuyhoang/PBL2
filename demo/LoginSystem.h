...
public:
    ...
    bool loadAllData() {
        bool s = true; s&=loadKhoaFromFile("Khoa.txt"); s&=loadMonHocFromFile("MonHoc.txt");
        s&=loadStudentFromFile("Student.txt"); s&=loadTeacherFromFile("Teacher.txt");
        s&=loadLopHPFromFile("LopHP.txt"); s&=loadDangKyFromFile("DangKy.txt"); s&=loadAccountsFromFile("Account.txt");
        return s;
    }
